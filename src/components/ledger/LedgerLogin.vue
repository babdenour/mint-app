<template>
  <div class="vue3rdj5__mode" v-if="openContent">
    <div class="vue3rdj5__mode-error" v-if="error" :error="error">{{ error }}</div>
    ...
    <div class="vue3rdj5__ledger">
      <ul class="vue3rdj5__ledger-list">
        <li class="vue3rdj5__ledger-item" v-for="(account, index) in accounts" :key="index">
          <a class="vue3rdj5__ledger-link" @click="login(startIndex + index)" href="#">{{ account }}</a>
        </li>
      </ul>
    </div>
  </div>
